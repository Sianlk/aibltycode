"""
AIBLTY Code — FastAPI Backend
Domain: developer tools
AI Focus: code generation, debugging, code review, documentation
Features: AI Code Complete, Bug Detection, Code Review, Docs Generator, Terminal AI
"""
...
async def lifespan(app: FastAPI):
    print(f"[AIBLTY Code] Starting up...")
    yield
    print(f"[AIBLTY Code] Shutting down...")

# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="AIBLTY Code API",
    description="AI coding assistant & pair programmer",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def root():
    return {
        "service": "AIBLTY Code",
        "tagline": "AI coding assistant & pair programmer",
        "domain": "developer tools",
        "version": "1.0.0",
        "docs": "/docs",
        "status": "live",
...
Focus area: code generation, debugging, code review, documentation.
Tagline: AI coding assistant & pair programmer.
Provide accurate, helpful, domain-specific responses."""
    ...
    if not openai_key:
        ...
        content = f"[DEV MODE] AIBLTY Code AI received: {request.prompt[:100]}"
...
async def websocket_ai(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_text(f"[AIBLTY Code AI] Processing: {data[:50]}")
