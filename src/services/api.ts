...
// Auth helpers
export const auth = {
  register: (email: string, password: string, fullName = "", appSlug = "aibltycode") =>
...
export const aiApi = {
  complete: (message: string, context?: Array<{ role: string; content: string }>) =>
    apiPost<{ content: string; model: string; duration_ms: number }>(
      "/api/ai/complete", { message, app_slug: "aibltycode", context }
    ),
  agent: (task: string, agentType = "analyst") =>
    apiPost<{ result: string; agent_type: string }>(
      "/api/ai/agent", { task, app_slug: "aibltycode", agent_type: agentType }
    ),
};
...
export const analyticsApi = {
  batch: (events: Array<{ event_name: string; properties?: Record<string, unknown> }>) =>
    apiFetch('/api/analytics/batch', {
      method: 'POST',
      body: JSON.stringify({
        events: events.map(e => ({ ...e, app_slug: "aibltycode" }))
      }),
    }),
};
